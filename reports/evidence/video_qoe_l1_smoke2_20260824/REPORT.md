...
- 采集时间：2026-08-25T09:57:50+08:00
- 采集时长：4.4 s
...
| 下行 P5 / P50 / P95 | 0.001 / 0.001 / 0.001 Mbit/s |
| 波动幅度 P95-P5 | 0.001 Mbit/s |
| 标准差 / 变异系数 CV | 0.000 Mbit/s / 0.330 |
| 5 秒窗口 P5 / P50 / P95 | 不可用 / 不可用 / 不可用 Mbit/s |
| Wi-Fi 信号均值 / 最差 | -70.0 / -70.0 dBm |

分片视频会呈现“突发下载—空闲缓存”节奏，因此 1 秒吞吐 CV 高不能单独判为卡顿；5 秒窗口更适合观察持续供给能力。

## 2. IP 层与接口丢弃

- CPE 蜂窝 WAN 主动探测：发送 5、接收 5，丢包率 **0.00%**。
- ICMP RTT 均值 / P95 / 相邻样本抖动：65.94 / 89.02 / 27.00 ms。
- 蜂窝接口 RX/TX dropped 增量：0/0；errors 增量：0/0。
- Wi-Fi 驱动 RX/TX dropped 增量：12/0。该 MTK 驱动计数可能包含无线失败/重试，不能直接当作端到端 IP 丢包率。
...
| 指标 | 结果 |
|---|---:|
| 被动 RTT 样本方法 / 数量 | tcp_ack_or_handshake / 0 |
| TCP RTT 均值 / P95 | 不可用 / 不可用 ms |
| TCP RTT 抖动（相邻 RTT 绝对差均值） | 不可用 |
| 目标终端 TCP 重传 | 0 次 |
| 目标终端 TCP 重传占 TCP 数据包比例 | 不可用 |
| 目标终端零窗口事件 | 0 次 |
| TCP 流数量 | 0 |

下行包头可见流量中 TCP 字节占比约 不可用；UDP/443（QUIC 候选）占比约 不可用。UDP/443 只按端口识别，不属于 DPI 业务识别。
...
- 原始包记录 0，解析后的目标 IP 包 0；修正 MTK/bridge tap 重复呈现 0 包。
- tcpdump 内核丢包：None。
