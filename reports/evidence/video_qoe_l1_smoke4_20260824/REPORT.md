...
- 采集时间：2026-08-25T10:04:05+08:00
- 采集时长：4.4 s
...
|---|---:|
| 下行平均吞吐 | 0.001 Mbit/s |
| 下行峰值吞吐（实际平均采样间隔 1.48 s） | 0.002 Mbit/s |
| 下行 P5 / P50 / P95 | 0.000 / 0.001 / 0.002 Mbit/s |
| 波动幅度 P95-P5 | 0.002 Mbit/s |
| 标准差 / 变异系数 CV | 0.001 Mbit/s / 0.881 |
| 5 秒窗口 P5 / P50 / P95 | 不可用 / 不可用 / 不可用 Mbit/s |
| Wi-Fi 信号均值 / 最差 | -65.0 / -65.0 dBm |
...
- ICMP RTT 均值 / P95 / 相邻样本抖动：75.98 / 101.30 / 40.75 ms。
- 蜂窝接口 RX/TX dropped 增量：0/0；errors 增量：0/0。
- Wi-Fi 驱动 RX/TX dropped 增量：8/0。该 MTK 驱动计数可能包含无线失败/重试，不能直接当作端到端 IP 丢包率。
